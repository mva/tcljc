...
public Object reduce(IFn f, Object start) {
    if(array != null) {
        var mh = IFn.__arity(f, 2);
        try {
            Object ret = mh.invoke(f, start, array[i]);
            for(int x = i + 1; x < array.length; x++)
                {
                    if(RT.isReduced(ret))
                        return ((IDeref)ret).deref();
...
static public class ArraySeq_float extends ASeq implements IndexedSeq, IReduce{
	...
	public Object reduce(IFn f) {
		...
		for(int x = i + 1; x < array.length; x++)
                    {
			ret = mh.invoke(f, ret, Numbers.num(array[x]));
			if(RT.isReduced(ret))
                            return ((IDeref)ret).deref();
                    }
		return ret;
	...
	public Object reduce(IFn f, Object start) {
            var mh = IFn.__arity(f, 2);
            try {
		Object ret = mh.invoke(f, start, Numbers.num(array[i]));
		for(int x = i + 1; x < array.length; x++)
                    {
			if(RT.isReduced(ret))
                            return ((IDeref)ret).deref();
